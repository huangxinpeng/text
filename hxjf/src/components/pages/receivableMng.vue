<!-- 池保理应收账款管理页面-->
<template>
  <div class="receivableMng" >
    <div class="" style="margin-bottom:20px;">
      <div class="row">
        <div class="col-md-10 col-md-offset-1">
          <div class="col-md-12 textHeight">
            <div class="col-md-12">
              <span class="h4">池保理>应收账款管理</span>
        ...
        <div class="col-md-10  col-md-offset-1">
          ...
          <div class="col-md-12">
            <div class="col-md-12">
              <router-link to="/poolDebtAdd" class="buttonl btn btn-warning">新增</router-link>


              <button type="button" class="buttonl btn btn-warning">模板下载</button>   
              <button type="button" class="buttonl btn btn-warning">账款导入</button>
            </div>
            <!-- ---- -->
...
export default {
  ...
  data(){
    return {
                ...
                cur: 1,//当前页码
                url:'',  // 地址
                product:'1230040',//产品类型(1230010-单保理；1230040-保理池)
                stotal:'', //后台总数
              aJson:[] , //接口的数据
              aJsons:[],// 修改的接口
              querys:{
          ...
          created:function(){
            ...
            this.url=urls
            var aJson=JSON.stringify({pageSize:10,pageNumber:1,product:this.product})
            this.$http.post( this.url, aJson, { headers:URL.headers },{emulateJSON:true}).then(function(response){
    ...
    if(this.aJson!=0){
      this.$http.get(delurl,{ headers:URL.headers },{
        emulateJSON:true
                    ...
                    this.$http.post(this.url,JSON.stringify({pageSize:10,pageNumber:1,product:this.product}), { headers:URL.headers }).then(function(response){
...
//页码
    btnClick: function(data){//页码点击事件
      if(data != this.cur){
        this.cur = data 
        this.$http.post(this.url,JSON.stringify({pageSize:10,pageNumber:this.cur,product:this.querys.product,billsNo:this.querys.billsNo,custcdBuyer:this.querys.custcdBuyer,beginDate:this.querys.beginDate}), { headers:URL.headers },{emulateJSON:true}).then(function(response){
...
query:function(){
  ...
  this.url=queryUirl
  this.$http.post(this.url,JSON.stringify(this.querys), { headers:URL.headers },{emulateJSON:true})
  .then(function(response){
    ...
    if(response.body.retcode==="202"){
      ...
      this.url=urls
      this.$http.post( this.url,{pageSize:10,pageNumber:1,product:this.product}, { headers:URL.headers },{emulateJSON:true})
      .then(function(response){
        this.aJson = response.data.rows
...
<style scoped>
.receivableMng{width: 100%;overflow: hidden;}
...
.buttonl{ margin-right: 25px;width: 100px;text-align: center;padding: 5px;border: none;margin-left: -15px;margin-top: -5px;margin-bottom: 10px; }
.w500{width: 500px;}
